...
async function create(req: Request, res: Response) {
    try {
      const userId = req.user?.id;
      const tasa = await createTasa(req.body, Number(userId));
...
async function update(req: Request, res: Response) {
    try {
      const userId = req.user?.id;
      const tasa = await updateTasa(req.body, Number(userId), Number(req.params.id));
...
async function remove(req: Request, res: Response) {
    try {
      const userId = req.user?.id;
      const tasa = await removeTasa(Number(userId), Number(req.params.id));
      res.status(204).send();
    } catch (error: any) {
      const status = error.status || 500;
      res.status(status).json({ message: error.message || 'Error interno' });    
